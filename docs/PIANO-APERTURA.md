...
- [x] ✅ **Cancellata `Ecosystem-Runtime/runtime-telegrambot-releases` il 13/08**, su conferma esplicita dell'utente, dopo il rilascio della v2.2.0. Verificato: la repo e il PDF italiano rispondono **HTTP 404**. Con lei se ne vanno gli 8 documenti col colophon EULA — il progetto MIT non ha più nulla di ufficiale che dichiari «Tutti i diritti riservati» — e la release v2.1.7 coi suoi 5 installer, che erano l'unica perdita reale e valeva zero (l'unica installazione al mondo è quella dell'autore, da tempo su versioni successive).
  > **Backup prima di procedere:** `git clone --mirror` in `_backup-repo-ponte-2026-08-13.git`, 36 MB, accanto agli altri backup nella cartella del progetto. Contiene storia, README, banner e tutti e 8 i PDF. **Non contiene gli installer della release**, che non stanno in git: quelli non sono recuperabili, ed era una perdita accettata.
  >
  > Le tre righe qui sotto sono il ragionamento che ha preceduto la decisione, lasciate come traccia.
...
## FASE 6 — Rifiniture della schermata iniziale ✅ CHIUSA
...
Il giudizio dell'utente sul resto della schermata è: **va bene così.** Non c'è mandato per ridisegnarla — e infatti non è stata toccata nient'altro.

**Eseguita e verificata il 13 agosto 2026.** Le quattro voci sono state misurate sulla schermata montata dal vivo, non solo lette nel diff: la Fase 6 è tutta geometria e contrasto, e su queste cose il codice che «sembra giusto» non basta.

- [x] **1. «Titan Edition» va portato a capo.** Il titolo era un unico `<h1>` in cui `{t('app.title')}` e lo `<span>` «Titan Edition» stavano sulla stessa riga, quindi il testo andava a capo dove capitava: nella schermata dell'utente si spezzava dopo «Titan», lasciando «Edition» orfano sulla seconda riga. Il nome canonico è composto di due parti, e la seconda deve stare sotto la prima per intero. **Fatto** con `block` sullo `<span>`, che rende il capoverso strutturale invece che dipendente dalla larghezza. Aggiunto anche `text-center` sull'`h1`: il centraggio veniva dal genitore e saltava appena il titolo andava su due righe. *Misurato:* `h1` alto 80px con `line-height` 40px — due righe esatte, `text-align: center`

- [x] **2. Il credito LLM è illeggibile.** «Sviluppato con l'ausilio di LLM · concezione e direzione di Simone Pizzi» era `text-nano` a `text-outline-variant/35`. È anche la riga di paternità del progetto, cioè una delle cose che l'apertura del sorgente doveva rendere esplicite — nasconderla è il contrario di quel che serve.

  ⚠️ **La diagnosi del piano era incompleta, e alzare opacità e corpo non sarebbe bastato.** Due scoperte fatte leggendo il codice invece che il piano. Primo: `.text-nano` ([index.css:139](../src/renderer/src/index.css)) non è solo un corpo di 9px, è anche `font-family: Fira Code` e `text-transform: uppercase`. La riga era **in maiuscoletto monospace**, e vinceva sul `font-body` scritto accanto. Secondo: `outline-variant` è `#3b82f6` ([tailwind.config.js:33](../tailwind.config.js)), blu pieno — «blu su blu» era letterale. Su fondo `#050510` quel token **non arriva a un contrasto accettabile a nessuna opacità utile**: al 35% dava 2,0:1, e perfino al 75% si fermava a 3,5:1, sotto il minimo AA di 4,5:1.

  **Fatto:** `text-xs` (12px, minuscolo, Inter) e il token `on-surface-variant` (`#bfdbfe`), che è quello già previsto dalla palette per il testo secondario su scuro. *Misurato:* **7,13:1**, da 2,0:1

- [x] **3. Le bandiere non sono centrate — ed è un residuo delle otto lingue.** Il contenitore era `grid grid-cols-4`: pensato per otto bandiere su due file da quattro. Con due lingue restavano incollate a sinistra, e il vuoto a destra lasciava intendere che *prima ci fosse dell'altro* — che è esattamente quel che è successo, ma non è quel che deve trasparire. **Fatto** con `flex flex-wrap justify-center` e `w-32` sui bottoni. Il `w-32` non è un numero scelto a occhio: 128px è esattamente la larghezza che ogni colonna aveva prima — `(560 − 3×16) / 4` — quindi i bottoni restano identici e cambia solo dove stanno. *Misurato:* 143px di margine a sinistra e 143px a destra, bottoni a 128px

- [x] **4. `INIT_SEQ · TITAN_DESKTOP_RUNTIME` — a che serve?** A niente: decorazione, un vezzo da finto terminale rimasto dal design «Titan Glass» delle origini. **Decisione dell'utente, 13/08: toglierla.** Rimossa; non era una chiave di traduzione e non compare in nessun altro punto del codice

### Due cose viste di sfuggita, non toccate

Nessuna delle due è Fase 6, e la regola di una fase per sessione vale anche per le tentazioni piccole.

- **La riga di copyright sopra il credito LLM sta a 2,6:1.** È `text-outline-variant/60`, lo stesso blu del problema n. 2, e ora è la riga meno leggibile della schermata — meno di quella che abbiamo appena sistemato. Il piano non la nominava e il mandato dell'utente sul resto della schermata è «va bene così», quindi è rimasta com'era. Ma il token `outline-variant` come colore di testo è un problema di palette, non di questa schermata: **vale la pena cercarlo altrove nell'interfaccia**
- **`npm run dev` non riesce ad avviare Electron.** In watch mode partono due build del processo main e la seconda ignora `lib.formats: ['cjs']`, scrivendo ESM dentro `index.cjs`; Electron si apre su un dialogo di errore. È il bug già descritto nei commenti di [vite.config.ts:33](../vite.config.ts) — il `watch.exclude` messo lì per romperlo **non basta**. Riguarda solo lo sviluppo: `vite build` produce un `index.cjs` corretto, verificato in questa sessione, quindi installer e CI non ne risentono
