...
- **起始日期** — 机器人将忽略此日期之前发布的所有内容。有助于避免用旧文章刷屏频道。
...
- **关键词过滤器 (F4)** — 按关键词过滤文章（包含或排除）。可在 feed 设置中启用。琥珀色徽章表示过滤器已激活。
- **自定义间隔 (F5)** — 为该 feed 设置独立的获取间隔（5 分钟至 24 小时），不受机器人全局间隔影响。
- **Digest Mode (F9)** — 不再逐篇发布文章，而是在可配置的时间段内（1小时、6小时、12小时、24小时、7天）积累内容，并以一条摘要消息发送。紫色徽章表示该模式已激活。
- **OPML 导入 (F8)** — 通过 Feed Manager 中的 OPML 按钮，从标准 `.opml` 文件一次性导入多个 feed。
...
- **预览 (F7)** 按钮可显示消息使用示例数据后的效果，无需离开编辑器。
