...
- [v2.3.0 (Corrente)](docs/changelogs/CHANGELOG_v2.3.0.md) — Il pannello **Sorgenti Feed** guadagna una lente di ricerca, locale al canale selezionato: su un canale affollato ritrovare un feed non è più un lavoro di scorrimento. Si cerca su nome, indirizzo, tipo e parole del filtro; più termini valgono come una condizione sola; un contatore dice quanti feed su quanti, così una ricerca troppo stretta non si confonde con un canale quasi vuoto. Si azzera con ✕ o `Esc`, e cambiando canale riparte pulita. Nessuna modifica al motore, ai feed o al database.
- [v2.2.1 (Archivio)](docs/changelogs/CHANGELOG_v2.2.1.md) — Correzione singola: nel diario di scansione finivano righe marcate come **errore** che errori non erano. Il livello di una riga si deduceva cercando «error» e «Fallito» in tutto il testo, titolo del contenuto compreso: due video con «Checked Errors» e «trial and error» nel titolo comparivano in rosso a ogni giro. Ora contano solo i marcatori del programma, e le sei segnalazioni del lettore YouTube che avvisavano a parole dichiarano il proprio livello, così non spariscono dal diario. Su una giornata di log: da 11 righe rosse a 1, l'unica vera.
- [v2.2.0 (Archivio)](docs/changelogs/CHANGELOG_v2.2.0.md) — Il pannello dei log della Dashboard diventa un **diario di scansione**: il motore dichiara nove tipi di evento invece di lasciarli dedurre dal testo, e a schermo restano solo gli eventi che contano più tutto ciò che è andato storto. La console di prima resta, dietro l'interruttore Diario/Console nell'intestazione del pannello. L'esportazione legge il file di log della giornata invece dell'array in memoria. L'indirizzo del canale e la data nella scelta del bot tornano leggibili.
- [v2.1.9 (Archivio)](docs/changelogs/CHANGELOG_v2.1.9.md) — Sola interfaccia, nessun cambiamento al motore. Schermata iniziale: «Titan Edition» non si spezza più a metà, il credito agli LLM e alla direzione del progetto passa da 2,0:1 a 7,13:1 di contrasto, bandiere centrate, scritta decorativa `INIT_SEQ` rimossa. Impostazioni di Sistema: la scheda «Generale» va su due colonne e il modale non esce più dallo schermo; le tre schede hanno tutte la stessa misura.
